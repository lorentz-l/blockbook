...
// TronRPC is an interface to JSON-RPC eth service.
type TronRPC struct {
	*bchain.BaseChain
...
// NewTronRPC returns new EthRPC instance.
...
func (b *TronRPC) subscribe(f func() (bchain.EVMClientSubscription, error)) error {
	s, err := f()
	if err != nil {
		return err
	}
...
func (b *TronRPC) getConsensusVersion() string {
...
func (b *TronRPC) UpdateBestHeader(h bchain.EVMHeader) {
...
func (b *TronRPC) ethHeaderToBlockHeader(h *rpcHeader) (*bchain.BlockHeader, error) {
...
func (b *TronRPC) processEventsForBlock(blockNumber string) (map[string][]*bchain.RpcLog, []bchain.AddressAliasRecord, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.Timeout)
	defer cancel()
...
func (b *TronRPC) getCreationContractInfo(contract string, height uint32) *bchain.ContractInfo {
...
func (b *TronRPC) processCallTrace(call *rpcCallTrace, d *bchain.EthereumInternalData, contracts []bchain.ContractInfo, blockHeight uint32) []bchain.ContractInfo {
...
func (b *TronRPC) getInternalDataForBlock(blockHash string, blockHeight uint32, transactions []bchain.RpcTransaction) ([]bchain.EthereumInternalData, []bchain.ContractInfo, error) {
